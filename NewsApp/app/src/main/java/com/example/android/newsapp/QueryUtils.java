    ...
    private static List<News> extractFeatureFromJson(String newsJSON) {
        ...
        try {
            ...
            for (int i = 0; i < newsArray.length(); i++) {
                ...
                //Create an array for tags that include article contributors
                JSONArray contributorArray = currentNews.getJSONArray("tags");
                String contributors = "";
                for (int j = 0; j < contributorArray.length(); j++) {

                    JSONObject tags = contributorArray.getJSONObject(j);
                    // Extract the value for the key called "web title"
                    if (j == 0) {
                        contributors = contributors + tags.getString("webTitle");
                    } else {
                        contributors = contributors + ", " + tags.getString("webTitle");
                    }
                }

                // Create a new {@link News} object with the sectionName, webTitle, contributor
                // and url from the JSON response.
                News newNews = new News(newsCategory, newsTitle, url, contributors);
